...
client.on('message', msg => {
  ...
  if (msg.content.toLowerCase().includes('lafmı soktun şimdi yarram')) msg.reply('soktum lan napcan yapram :smile:');
  if (msg.content.toLowerCase().includes('ya havle')) msg.reply('vela kuvvete :smile: :joy:');
  if (msg.content.toLowerCase().includes('knk sen benim sabrımımı deniyon ?')) msg.reply('Bilmem deniyommu. Deniyom lan napcan heğ ? :smile: :joy:')
});
...
client.on('guildMemberAdd', async(member) => {
    let mute = member.guild.roles.cache.find(r => r.name === "Muteli Şahıslar");
    let muterol = member.guild.roles.cache.find(r => r.id === '851637492590444544')
    ...
    db.delete(`muteli_${member.guild.id + member.id}`)
        ...
        member.roles.remove('849908028504670268');
      }, ms(süre));
    }
    })
client.on("message", async msg => {//cringeengel komutları
    if(msg.author.bot) return;
    if(msg.channel.type === "dm") return;
        
    let i = await db.fetch(`cringeFiltre_${msg.guild.id}`) 
          if (i == 'acik') {
              const cringe = ["hİh", "hih", "h1h", "hIh", "napim", "Na1im", "na1im", "domal", "d0mal", "D0mal", "hıh", "küs benle", "-,-", "dömel", "hıh k1stüm", "d!mal", "küstüm", "k1stüm"];
              if (cringe.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("ADMINISTRATOR")) {
                    msg.delete();                   
                    let embed = new Discord.MessageEmbed()
                    .setColor(0xffa300)
                    .setFooter('  Cringe Engel.', client.user.avatarURL())
                    .setAuthor(msg.guild.owner.user.username, msg.guild.owner.user.avatarURL())
                    .setDescription("Cringe sistemi " + `***${msg.guild.name}***` + " adlı sunucunuzda cringe birini yakaladım")
                    .addField('cringe kişi', 'Kullanıcı: '+ msg.author.tag +'\nID: '+ msg.author.id, true)
                    .addField('Engellenen mesaj', msg.content, true)
                    .setTimestamp()                   
                    msg.guild.owner.user.send(embed)            
                    return msg.channel.send(`${msg.author.tag}, <:galp:849897748090060841> Çok cringesin **yapma**`).then(msg => msg.delete(25000));
                  }             
                } catch(err) {
                  console.log(err);
                }
...
const webhook = new Discord.WebhookClient('852557931416125462','01Ov9ie9vEahO9LyBw_ItAioeXyd0it5PHas11WEfzp9WcD-BrsbmLWRFi66Ok4iAjQs');
